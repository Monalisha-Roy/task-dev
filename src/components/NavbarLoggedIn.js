export default function NavbarLoggedIn({ onAddPostClick, onLogoutClick, username, email }) {
  ...
  return (
    <nav className="w-full px-6 py-2 bg-gray-600 flex justify-between items-center">
      ...
      <div className="flex-col justify-start items-start p-2 w-2/5 rounded-md bg-green-400">
        <p className="text-white text-lg px-5">Username: {username}</p>
        <p className="text-white text-lg px-5">Email: {email}</p>
      </div>
